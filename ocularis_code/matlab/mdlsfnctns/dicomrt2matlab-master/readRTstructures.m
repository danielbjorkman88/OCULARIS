function [contours,xfm] = readRTstructures(rtssheader, imgheaders)
  ...
  try
    ...
    for j = 1:length(ContourSequence)
      ...
      if strcmp(rtssheader.ROIContourSequence.(ROIContourSequence{i}).ContourSequence.(ContourSequence{j}).ContourGeometricType, 'CLOSED_PLANAR')
        ...
        if (minvox(3):maxvox(3)) == 0
            continue
        end

        contours(i).Segmentation((minvox(1):maxvox(1))+1, (minvox(2):maxvox(2))+1, (minvox(3):maxvox(3))) = permute(reshape(in, size(x)), [2 1]);
